...
type SGDState{T<:Real,N} <: ParameterUpdaterState
    v::Array{T,N}
end
SGDState(n::Int) = SGDState(zeros(n))
SGDState{N}(dims::NTuple{N,Int}) = SGDState(zeros(dims))
SGDState{N}(gtype::Type,dims::NTuple{N,Int}) = SGDState(zeros(gtype,dims))

function param_update!(state::SGDState, updater::SGDUpdater, gradient::AbstractArray)
    @_dimcheck size(state.v) == size(gradient)
    for i in eachindex(gradient)
        @inbounds param_update!(state, updater, gradient[i], i)
    end
end

@inline function param_update!(state::SGDState, updater::SGDUpdater, gradient::Real, idx)
    state.v[idx] = -updater.η * gradient + updater.μ * state.v[idx]
end
